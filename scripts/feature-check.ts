...
console.log("\n— delivery to the door (0046) —");
{
  ...
  check("a blank spot is fine — the runner calls", deliveryProblem({ spot: " ", detail: "", phone: "9876543210" }), null);
  check("a spot in the student's own words is fine", deliveryProblem({ spot: "Block C, second floor", detail: "", phone: "9876543210" }), null);
  check("a spot without a detail is fine", deliveryProblem({ spot: "Library entrance", detail: "", phone: "9876543210" }), null);
  check("a long spot is refused", deliveryProblem({ spot: "x".repeat(61), detail: "", phone: "9876543210" })?.includes("short"), true);
